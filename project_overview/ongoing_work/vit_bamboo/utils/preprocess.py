...
def calculate_top_k_accuracy(prob_matrix, k_values=[1, 5, 10, 15, 20, 50]):
    n = prob_matrix.shape[0]
    accuracies = {k: 0 for k in k_values}
    
    for row_idx in range(n):
        row_sum = prob_matrix[row_idx].sum()
        if row_sum == 0:
            row_probs = np.ones_like(prob_matrix[row_idx]) / n
        else:
            row_probs = prob_matrix[row_idx] / row_sum
        
        sorted_indices = np.argsort(row_probs)[::-1]
        target_col_idx = row_idx
        target_rank = np.where(sorted_indices == target_col_idx)[0][0]
