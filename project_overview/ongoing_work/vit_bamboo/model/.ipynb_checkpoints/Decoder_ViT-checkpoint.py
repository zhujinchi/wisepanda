...
class DecoderV2(nn.Module):
    def __init__(self, config):
        print("ViT_Only: DecoderV2")
        super().__init__()
        
        model_config = config['model']
        
        vit_c, vit_h, vit_w  = config['basemodel']['vit']['vit_output_shape']
        self.num_c    = vit_c
        self.feat_h   = vit_h
        self.feat_w   = vit_w
    ...
    def forward(self, dino_feat:torch.Tensor, base_feat:torch.Tensor):
        batch_size, c_num, feat_h, feat_w = base_feat.shape
        assert(self.num_c   == c_num)
        assert(self.feat_h  == feat_h)
        assert(self.feat_w  == feat_w)
        
        x = base_feat.reshape([batch_size, c_num, -1]).permute(0,2,1)
        x = self.first_norm(x)
        x = self.dim_red(x)
